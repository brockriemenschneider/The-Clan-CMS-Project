...
class Events extends CI_Controller {
	...
	{
		// Convert arguments to db usable params
		$data = explode('-', $month);
		$month = $data[0];
		$year = '20'.$data[1];
		...
		if($events = $this->events->get_events(array('event_month' => $month, 'event_year' => $year)))
		{
			...
			foreach($events as $entry)
			{
				...
				$event[$entry->event_day] = array('events/' . lcfirst(date("M-y", mktime(0, 0, 0, $entry->event_month))) . '/' . $entry->event_slug, 'calendar-event', $entry->event_title);
		...
		$prev_next = array(
		 ...
		 // Create the calendar
		$calendar = $this->calendar->clancms_calendar($today, $year,  $month, (isset($event) ? $event : ''), 1, lcfirst(date('M-y', $time)), 0, $prev_next);
			
		
		if($add_event = $this->input->post('add_event'))
		{
